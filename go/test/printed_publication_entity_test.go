...
func TestPrintedPublicationEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.PrintedPublication(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.PrintedPublication(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		printedPublicationRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.printed_publication")))
		...
		printedPublicationRef01DataDt0LoadResult := core.ToMapAny(entityData(printedPublicationRef01DataDt0Loaded))
...
func printed_publicationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ART_INSTITUTE_OF_CHICAGO_TEST_PRINTED_PUBLICATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ART_INSTITUTE_OF_CHICAGO_TEST_PRINTED_PUBLICATION_ENTID"])
