...
func TestAgentDirect(t *testing.T) {
	t.Run("direct-list-agent", func(t *testing.T) {
		setup := agentDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-agent", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-agent", func(t *testing.T) {
		setup := agentDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-agent", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type agentDirectSetupResult struct {
...
func agentDirectSetup(mockres any) *agentDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"ART_INSTITUTE_OF_CHICAGO_TEST_AGENT_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["ART_INSTITUTE_OF_CHICAGO_TEST_AGENT_ENTID"]; ok {
		...
		return &agentDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &agentDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
