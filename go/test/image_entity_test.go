...
func TestImageEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set ARTINSTITUTEOFCHICAGO_TEST_IMAGE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func imageBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ARTINSTITUTEOFCHICAGO_TEST_IMAGE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ARTINSTITUTEOFCHICAGO_TEST_IMAGE_ENTID"])
