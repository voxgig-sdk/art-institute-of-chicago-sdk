...
func TestVideoEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Video(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Video(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set ART_INSTITUTE_OF_CHICAGO_TEST_VIDEO_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		videoRef01Match := map[string]any{}

		videoRef01ListResult, err := videoRef01Ent.List(videoRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, videoRef01ListOk := videoRef01ListResult.([]any)
		if !videoRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", videoRef01ListResult)
		}

		// LOAD
		videoRef01MatchDt0 := map[string]any{
			"id": videoRef01Data["id"],
		}
		videoRef01DataDt0Loaded, err := videoRef01Ent.Load(videoRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		videoRef01DataDt0LoadResult := core.ToMapAny(entityData(videoRef01DataDt0Loaded))
		if videoRef01DataDt0LoadResult == nil {
			t.Fatal("expected load result to be a map")
		}
		if videoRef01DataDt0LoadResult["id"] != videoRef01Data["id"] {
...
func videoBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ART_INSTITUTE_OF_CHICAGO_TEST_VIDEO_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ART_INSTITUTE_OF_CHICAGO_TEST_VIDEO_ENTID"])
