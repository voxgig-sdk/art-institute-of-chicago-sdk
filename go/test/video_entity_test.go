...
func TestVideoEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set ARTINSTITUTEOFCHICAGO_TEST_VIDEO_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		videoRef01DataDt0LoadResult := core.ToMapAny(videoRef01DataDt0Loaded)
...
func videoBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ARTINSTITUTEOFCHICAGO_TEST_VIDEO_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ARTINSTITUTEOFCHICAGO_TEST_VIDEO_ENTID"])
