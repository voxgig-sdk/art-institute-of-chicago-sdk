...
func TestProductDirect(t *testing.T) {
	...
	t.Run("direct-load-product", func(t *testing.T) {
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-product", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
func productDirectSetup(mockres any) *productDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"ART_INSTITUTE_OF_CHICAGO_TEST_PRODUCT_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["ART_INSTITUTE_OF_CHICAGO_TEST_PRODUCT_ENTID"]; ok {
