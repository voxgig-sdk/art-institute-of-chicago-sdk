...
func TestEventOccurrenceEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.EventOccurrence(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.EventOccurrence(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		eventOccurrenceRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.event_occurrence")))
		...
		eventOccurrenceRef01DataDt0LoadResult := core.ToMapAny(entityData(eventOccurrenceRef01DataDt0Loaded))
...
func event_occurrenceBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ART_INSTITUTE_OF_CHICAGO_TEST_EVENT_OCCURRENCE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ART_INSTITUTE_OF_CHICAGO_TEST_EVENT_OCCURRENCE_ENTID"])
