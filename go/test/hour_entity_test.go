...
func TestHourEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Hour(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Hour(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		hourRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.hour")))
		...
		hourRef01DataDt0LoadResult := core.ToMapAny(entityData(hourRef01DataDt0Loaded))
...
func hourBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ART_INSTITUTE_OF_CHICAGO_TEST_HOUR_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ART_INSTITUTE_OF_CHICAGO_TEST_HOUR_ENTID"])
