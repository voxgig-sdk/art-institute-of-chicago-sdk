...
//
// NewArtInstituteOfChicagoSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.ArtInstituteOfChicagoSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewArtInstituteOfChicagoSDK(opts)
}
...
func fcCandidates(client *sdk.ArtInstituteOfChicagoSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.ArtInstituteOfChicagoSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.ArtInstituteOfChicagoSDK, name string) any {
			...
			for _, raw := range cases {
				...
				for _, rawstep := range steps {
					...
					if code, ok := wanterr.(string); ok {
						...
						// the word.
						got := ""
						var sdkerr *sdk.ArtInstituteOfChicagoError
