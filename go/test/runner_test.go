...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("ART_INSTITUTE_OF_CHICAGO_TEST_LIVE") == "TRUE" ||
		os.Getenv("ART_INSTITUTE_OF_CHICAGO_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("ART_INSTITUTE_OF_CHICAGO_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.ArtInstituteOfChicagoSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.ArtInstituteOfChicagoError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.ArtInstituteOfChicagoSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.ArtInstituteOfChicagoError{Msg: msg, Code: code}
}
