...
func TestAgentEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set ARTINSTITUTEOFCHICAGO_TEST_AGENT_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		agentRef01DataDt0LoadResult := core.ToMapAny(agentRef01DataDt0Loaded)
...
func agentBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ARTINSTITUTEOFCHICAGO_TEST_AGENT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ARTINSTITUTEOFCHICAGO_TEST_AGENT_ENTID"])
