...
func TestDigitalPublicationEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set ARTINSTITUTEOFCHICAGO_TEST_DIGITAL_PUBLICATION_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		digitalPublicationRef01DataDt0LoadResult := core.ToMapAny(digitalPublicationRef01DataDt0Loaded)
...
func digital_publicationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ARTINSTITUTEOFCHICAGO_TEST_DIGITAL_PUBLICATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ARTINSTITUTEOFCHICAGO_TEST_DIGITAL_PUBLICATION_ENTID"])
