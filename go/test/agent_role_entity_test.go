...
func TestAgentRoleEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.AgentRole(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.AgentRole(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		agentRoleRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.agent_role")))
		...
		agentRoleRef01DataDt0LoadResult := core.ToMapAny(entityData(agentRoleRef01DataDt0Loaded))
...
func agent_roleBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ART_INSTITUTE_OF_CHICAGO_TEST_AGENT_ROLE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ART_INSTITUTE_OF_CHICAGO_TEST_AGENT_ROLE_ENTID"])
