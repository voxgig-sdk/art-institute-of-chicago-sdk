...
func TestSearchEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set ART_INSTITUTE_OF_CHICAGO_TEST_SEARCH_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func searchBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ART_INSTITUTE_OF_CHICAGO_TEST_SEARCH_ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"ART_INSTITUTE_OF_CHICAGO_TEST_SEARCH_ENTID": idmap,
		"ART_INSTITUTE_OF_CHICAGO_TEST_LIVE":      "FALSE",
		"ART_INSTITUTE_OF_CHICAGO_TEST_EXPLAIN":   "FALSE",
	})

	idmapResolved := core.ToMapAny(env["ART_INSTITUTE_OF_CHICAGO_TEST_SEARCH_ENTID"])
