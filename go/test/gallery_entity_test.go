...
func TestGalleryEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Gallery(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Gallery(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		galleryRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.gallery")))
		...
		galleryRef01DataDt0LoadResult := core.ToMapAny(entityData(galleryRef01DataDt0Loaded))
...
func galleryBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ART_INSTITUTE_OF_CHICAGO_TEST_GALLERY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ART_INSTITUTE_OF_CHICAGO_TEST_GALLERY_ENTID"])
