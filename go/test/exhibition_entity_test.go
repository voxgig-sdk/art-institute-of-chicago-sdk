...
func TestExhibitionEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Exhibition(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Exhibition(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		exhibitionRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.exhibition")))
		...
		exhibitionRef01DataDt0LoadResult := core.ToMapAny(entityData(exhibitionRef01DataDt0Loaded))
...
func exhibitionBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ART_INSTITUTE_OF_CHICAGO_TEST_EXHIBITION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ART_INSTITUTE_OF_CHICAGO_TEST_EXHIBITION_ENTID"])
