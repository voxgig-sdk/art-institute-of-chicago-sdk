...
import (
	...
	omni "github.com/voxgig-sdk/art-institute-of-chicago-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ArtInstituteOfChicagoError); is {
				out := map[string]any{
					"name":    "ArtInstituteOfChicagoError",
...
func omniCtx(arg any, client *sdk.ArtInstituteOfChicagoSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ArtInstituteOfChicagoSDK); is {
			client = live
