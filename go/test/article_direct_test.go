...
func TestArticleDirect(t *testing.T) {
	t.Run("direct-list-article", func(t *testing.T) {
		setup := articleDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-article", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-article", func(t *testing.T) {
		setup := articleDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-article", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type articleDirectSetupResult struct {
...
func articleDirectSetup(mockres any) *articleDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"ART_INSTITUTE_OF_CHICAGO_TEST_ARTICLE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["ART_INSTITUTE_OF_CHICAGO_TEST_ARTICLE_ENTID"]; ok {
		...
		return &articleDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &articleDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
