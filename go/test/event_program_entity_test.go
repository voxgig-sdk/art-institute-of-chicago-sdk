...
func TestEventProgramEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.EventProgram(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.EventProgram(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		eventProgramRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.event_program")))
		...
		eventProgramRef01DataDt0LoadResult := core.ToMapAny(entityData(eventProgramRef01DataDt0Loaded))
...
func event_programBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ART_INSTITUTE_OF_CHICAGO_TEST_EVENT_PROGRAM_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ART_INSTITUTE_OF_CHICAGO_TEST_EVENT_PROGRAM_ENTID"])
