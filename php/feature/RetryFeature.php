...
// ArtInstituteOfChicago SDK retry feature
...
class ArtInstituteOfChicagoRetryFeature extends ArtInstituteOfChicagoBaseFeature
{
    ...
    public function init(ArtInstituteOfChicagoContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ArtInstituteOfChicagoContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ArtInstituteOfChicagoContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ArtInstituteOfChicagoContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
