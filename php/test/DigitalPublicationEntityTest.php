...
class DigitalPublicationEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->DigitalPublication(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->DigitalPublication(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ART_INSTITUTE_OF_CHICAGO_TEST_DIGITAL_PUBLICATION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $digital_publication_ref01_data_dt0_load_result = Helpers::to_map(is_object($digital_publication_ref01_data_dt0_loaded) && method_exists($digital_publication_ref01_data_dt0_loaded, 'data_get') ? $digital_publication_ref01_data_dt0_loaded->data_get() : $digital_publication_ref01_data_dt0_loaded);
...
function digital_publication_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("ART_INSTITUTE_OF_CHICAGO_TEST_DIGITAL_PUBLICATION_ENTID");
