...
class AgentEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ARTINSTITUTEOFCHICAGO_TEST_AGENT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $agent_ref01_data = null;
        if (count($agent_ref01_data_raw) > 0) {
        ...
        $this->assertNull($err);
        $agent_ref01_data_dt0_load_result = Helpers::to_map($agent_ref01_data_dt0_loaded);
...
function agent_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["agent01", "agent02", "agent03"] as $k) {
    ...
    $entid_env_raw = getenv("ARTINSTITUTEOFCHICAGO_TEST_AGENT_ENTID");
