...
class StaticPageEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ARTINSTITUTEOFCHICAGO_TEST_STATIC_PAGE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        [$static_page_ref01_list_result, $err] = $static_page_ref01_ent->list($static_page_ref01_match, null);
        $this->assertNull($err);
        ...
        [$static_page_ref01_data_dt0_loaded, $err] = $static_page_ref01_ent->load($static_page_ref01_match_dt0, null);
        $this->assertNull($err);
...
function static_page_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("ARTINSTITUTEOFCHICAGO_TEST_STATIC_PAGE_ENTID");
