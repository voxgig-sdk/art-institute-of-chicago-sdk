...
class DigitalPublicationArticleDirectTest extends TestCase
{
    public function test_direct_list_digital_publication_article(): void
    {
        $setup = digital_publication_article_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-digital_publication_article", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
    ...
    public function test_direct_load_digital_publication_article(): void
    {
        $setup = digital_publication_article_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-digital_publication_article", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
