...
class ExhibitionEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = ArtInstituteOfChicagoSDK::test(null, null);
        $ent = $testsdk->Exhibition(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Exhibition(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Exhibition(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = exhibition_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "exhibition." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ART_INSTITUTE_OF_CHICAGO_TEST_EXHIBITION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $exhibition_ref01_data = null;
        if (count($exhibition_ref01_data_raw) > 0) {
        ...
        $exhibition_ref01_data_dt0_load_result = Helpers::to_map(is_object($exhibition_ref01_data_dt0_loaded) && method_exists($exhibition_ref01_data_dt0_loaded, 'data_get') ? $exhibition_ref01_data_dt0_loaded->data_get() : $exhibition_ref01_data_dt0_loaded);
...
function exhibition_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/exhibition/ExhibitionTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["exhibition01", "exhibition02", "exhibition03"] as $k) {
    ...
    $entid_env_raw = getenv("ART_INSTITUTE_OF_CHICAGO_TEST_EXHIBITION_ENTID");
