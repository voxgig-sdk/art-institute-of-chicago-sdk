...
class GalleryEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Gallery(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Gallery(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = gallery_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "gallery." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ART_INSTITUTE_OF_CHICAGO_TEST_GALLERY_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $gallery_ref01_data = null;
        if (count($gallery_ref01_data_raw) > 0) {
        ...
        $gallery_ref01_data_dt0_load_result = Helpers::to_map(is_object($gallery_ref01_data_dt0_loaded) && method_exists($gallery_ref01_data_dt0_loaded, 'data_get') ? $gallery_ref01_data_dt0_loaded->data_get() : $gallery_ref01_data_dt0_loaded);
...
function gallery_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/gallery/GalleryTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["gallery01", "gallery02", "gallery03"] as $k) {
    ...
    $entid_env_raw = getenv("ART_INSTITUTE_OF_CHICAGO_TEST_GALLERY_ENTID");
