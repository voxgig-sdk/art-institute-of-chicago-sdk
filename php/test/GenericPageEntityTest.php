...
class GenericPageEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = ArtInstituteOfChicagoSDK::test(null, null);
        $ent = $testsdk->GenericPage(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->GenericPage(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->GenericPage(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = generic_page_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "generic_page." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $generic_page_ref01_data = null;
        if (count($generic_page_ref01_data_raw) > 0) {
            $generic_page_ref01_data = Helpers::to_map($generic_page_ref01_data_raw[0][1]);
        }

        // LIST
        $generic_page_ref01_ent = $client->GenericPage(null);
        $generic_page_ref01_match = [];

        $generic_page_ref01_list_result = $generic_page_ref01_ent->list($generic_page_ref01_match, null);
        $this->assertIsArray($generic_page_ref01_list_result);
...
function generic_page_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/generic_page/GenericPageTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["generic_page01", "generic_page02", "generic_page03"] as $k) {
