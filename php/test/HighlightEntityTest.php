...
class HighlightEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = ArtInstituteOfChicagoSDK::test(null, null);
        $ent = $testsdk->Highlight(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Highlight(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Highlight(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = highlight_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "highlight." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ART_INSTITUTE_OF_CHICAGO_TEST_HIGHLIGHT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $highlight_ref01_data = null;
        if (count($highlight_ref01_data_raw) > 0) {
        ...
        $highlight_ref01_data_dt0_load_result = Helpers::to_map(is_object($highlight_ref01_data_dt0_loaded) && method_exists($highlight_ref01_data_dt0_loaded, 'data_get') ? $highlight_ref01_data_dt0_loaded->data_get() : $highlight_ref01_data_dt0_loaded);
...
function highlight_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/highlight/HighlightTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["highlight01", "highlight02", "highlight03"] as $k) {
    ...
    $entid_env_raw = getenv("ART_INSTITUTE_OF_CHICAGO_TEST_HIGHLIGHT_ENTID");
