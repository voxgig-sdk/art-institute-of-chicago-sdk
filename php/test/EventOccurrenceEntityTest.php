...
class EventOccurrenceEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ARTINSTITUTEOFCHICAGO_TEST_EVENT_OCCURRENCE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        [$event_occurrence_ref01_list_result, $err] = $event_occurrence_ref01_ent->list($event_occurrence_ref01_match, null);
        $this->assertNull($err);
        ...
        [$event_occurrence_ref01_data_dt0_loaded, $err] = $event_occurrence_ref01_ent->load($event_occurrence_ref01_match_dt0, null);
        $this->assertNull($err);
...
function event_occurrence_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("ARTINSTITUTEOFCHICAGO_TEST_EVENT_OCCURRENCE_ENTID");
