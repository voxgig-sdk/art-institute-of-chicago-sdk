...
The Ruby SDK for the ArtInstituteOfChicago API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Agent(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.ArtInstituteOfChicago(nil).load(
  { "id" => "test01" }, nil
)
