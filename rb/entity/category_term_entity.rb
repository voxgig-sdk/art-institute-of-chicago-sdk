...
class CategoryTermEntity
  ...
  def make
    opts = @_entopts.dup
    CategoryTermEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.CategoryTerm.load works with no args).
  ...
  #
  # @param reqmatch [CategoryTermListMatch, Hash, nil] match filter (any subset of
  #   CategoryTerm fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
