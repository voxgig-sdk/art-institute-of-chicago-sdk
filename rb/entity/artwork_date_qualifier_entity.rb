...
class ArtworkDateQualifierEntity
  ...
  def make
    opts = @_entopts.dup
    ArtworkDateQualifierEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.ArtworkDateQualifier.load works with no args).
  ...
  #
  # @param reqmatch [ArtworkDateQualifierListMatch, Hash, nil] match filter (any subset of
  #   ArtworkDateQualifier fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
