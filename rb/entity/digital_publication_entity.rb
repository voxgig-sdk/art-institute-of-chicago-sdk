...
class DigitalPublicationEntity
  ...
  def make
    opts = @_entopts.dup
    DigitalPublicationEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.DigitalPublication.load works with no args).
  ...
  #
  # @param reqmatch [DigitalPublicationListMatch, Hash, nil] match filter (any subset of
  #   DigitalPublication fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
