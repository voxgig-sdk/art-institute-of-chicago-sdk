...
class EducatorResourceEntity
  ...
  def make
    opts = @_entopts.dup
    EducatorResourceEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.EducatorResource.load works with no args).
  ...
  #
  # @param reqmatch [EducatorResourceListMatch, Hash, nil] match filter (any subset of
  #   EducatorResource fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
