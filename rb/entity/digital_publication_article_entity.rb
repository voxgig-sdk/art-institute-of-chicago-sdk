...
class DigitalPublicationArticleEntity
  def initialize(client, entopts = nil)
    ...
    @_name = "digital_publication_article"
    @_client = client
  ...
  def make
    opts = @_entopts.dup
    DigitalPublicationArticleEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ArtInstituteOfChicagoHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [DigitalPublicationArticle, Hash] the current DigitalPublicationArticle data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = ArtInstituteOfChicagoHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of DigitalPublicationArticle fields)
  def match_get
  ...
  #
  # @param reqmatch [DigitalPublicationArticleLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.DigitalPublicationArticle.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [DigitalPublicationArticle, Hash] the loaded DigitalPublicationArticle; raises ArtInstituteOfChicagoError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = ArtInstituteOfChicagoHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [DigitalPublicationArticleListMatch, Hash, nil] match filter (any subset of
  #   DigitalPublicationArticle fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<DigitalPublicationArticle>, Array] the matching DigitalPublicationArticle items; raises ArtInstituteOfChicagoError on failure
