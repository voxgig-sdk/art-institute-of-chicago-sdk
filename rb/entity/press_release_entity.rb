...
class PressReleaseEntity
  ...
  def make
    opts = @_entopts.dup
    PressReleaseEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.PressRelease.load works with no args).
  ...
  #
  # @param reqmatch [PressReleaseListMatch, Hash, nil] match filter (any subset of
  #   PressRelease fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
