...
class ArticleEntity
  ...
  def make
    opts = @_entopts.dup
    ArticleEntity.new(@_client, opts)
  end
  ...
  #
  # @param reqmatch [ArticleLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Article.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Article, Hash] the loaded Article; raises ArtInstituteOfChicagoError on failure
  ...
  #
  # @param reqmatch [ArticleListMatch, Hash, nil] match filter (any subset of
  #   Article fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Article>, Array] the matching Article items; raises ArtInstituteOfChicagoError on failure
