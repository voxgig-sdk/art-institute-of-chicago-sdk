...
class SectionEntity
  ...
  def make
    opts = @_entopts.dup
    SectionEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.Section.load works with no args).
  ...
  #
  # @param reqmatch [SectionListMatch, Hash, nil] match filter (any subset of
  #   Section fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
