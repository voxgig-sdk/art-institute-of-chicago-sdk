...
class AgentRoleEntity
  ...
  def make
    opts = @_entopts.dup
    AgentRoleEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.AgentRole.load works with no args).
  ...
  #
  # @param reqmatch [AgentRoleListMatch, Hash, nil] match filter (any subset of
  #   AgentRole fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
