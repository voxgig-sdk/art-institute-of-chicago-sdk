...
class AgentEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Agent(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Agent(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set ARTINSTITUTEOFCHICAGO_TEST_AGENT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def agent_basic_setup(extra)
  ...
  entid_env_raw = ENV["ARTINSTITUTEOFCHICAGO_TEST_AGENT_ENTID"]
