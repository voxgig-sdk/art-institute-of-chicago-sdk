...
class AgentDirectTest < Minitest::Test
  def test_direct_list_agent
    setup = agent_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-agent", setup[:live] ? "live" : "unit")
    if _should_skip
  ...
  def test_direct_load_agent
    setup = agent_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-agent", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
...
def agent_direct_setup(mockres)
