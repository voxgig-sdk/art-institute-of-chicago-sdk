...
class HourDirectTest < Minitest::Test
  def test_direct_list_hour
    setup = hour_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-hour", setup[:live] ? "live" : "unit")
    if _should_skip
  ...
  def test_direct_load_hour
    setup = hour_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-hour", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
