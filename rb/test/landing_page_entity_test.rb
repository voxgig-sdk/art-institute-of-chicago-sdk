...
class LandingPageEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set ART_INSTITUTE_OF_CHICAGO_TEST_LANDING_PAGE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    landing_page_ref01_data_dt0_load_result = Helpers.to_map(landing_page_ref01_data_dt0_loaded.respond_to?(:data_get) ? landing_page_ref01_data_dt0_loaded.data_get : landing_page_ref01_data_dt0_loaded)
...
def landing_page_basic_setup(extra)
  ...
  entid_env_raw = ENV["ART_INSTITUTE_OF_CHICAGO_TEST_LANDING_PAGE_ENTID"]
