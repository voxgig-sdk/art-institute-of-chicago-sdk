...
class PublicationEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set ART_INSTITUTE_OF_CHICAGO_TEST_PUBLICATION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    publication_ref01_data_dt0_load_result = Helpers.to_map(publication_ref01_data_dt0_loaded.respond_to?(:data_get) ? publication_ref01_data_dt0_loaded.data_get : publication_ref01_data_dt0_loaded)
...
def publication_basic_setup(extra)
  ...
  entid_env_raw = ENV["ART_INSTITUTE_OF_CHICAGO_TEST_PUBLICATION_ENTID"]
