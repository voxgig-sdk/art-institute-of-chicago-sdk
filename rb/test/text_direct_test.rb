...
class TextDirectTest < Minitest::Test
  def test_direct_list_text
    setup = text_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-text", setup[:live] ? "live" : "unit")
    if _should_skip
  ...
  def test_direct_load_text
    setup = text_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-text", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
