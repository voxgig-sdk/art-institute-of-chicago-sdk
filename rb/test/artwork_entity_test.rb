...
class ArtworkEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Artwork(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Artwork(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = artwork_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "artwork." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set ART_INSTITUTE_OF_CHICAGO_TEST_ARTWORK_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    artwork_ref01_data = nil
    if artwork_ref01_data_raw.length > 0
    ...
    artwork_ref01_data_dt0_load_result = Helpers.to_map(artwork_ref01_data_dt0_loaded.respond_to?(:data_get) ? artwork_ref01_data_dt0_loaded.data_get : artwork_ref01_data_dt0_loaded)
...
def artwork_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "artwork", "ArtworkTestData.json")
  ...
  entid_env_raw = ENV["ART_INSTITUTE_OF_CHICAGO_TEST_ARTWORK_ENTID"]
