...
class DigitalPublicationArticleEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.DigitalPublicationArticle(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.DigitalPublicationArticle(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set ART_INSTITUTE_OF_CHICAGO_TEST_DIGITAL_PUBLICATION_ARTICLE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    digital_publication_article_ref01_data_dt0_load_result = Helpers.to_map(digital_publication_article_ref01_data_dt0_loaded.respond_to?(:data_get) ? digital_publication_article_ref01_data_dt0_loaded.data_get : digital_publication_article_ref01_data_dt0_loaded)
...
def digital_publication_article_basic_setup(extra)
  ...
  entid_env_raw = ENV["ART_INSTITUTE_OF_CHICAGO_TEST_DIGITAL_PUBLICATION_ARTICLE_ENTID"]
