...
class HighlightEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set ART_INSTITUTE_OF_CHICAGO_TEST_HIGHLIGHT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    highlight_ref01_data_dt0_load_result = Helpers.to_map(highlight_ref01_data_dt0_loaded.respond_to?(:data_get) ? highlight_ref01_data_dt0_loaded.data_get : highlight_ref01_data_dt0_loaded)
...
def highlight_basic_setup(extra)
  ...
  entid_env_raw = ENV["ART_INSTITUTE_OF_CHICAGO_TEST_HIGHLIGHT_ENTID"]
