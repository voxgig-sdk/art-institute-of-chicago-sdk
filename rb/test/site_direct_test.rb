...
class SiteDirectTest < Minitest::Test
  def test_direct_list_site
    setup = site_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-site", setup[:live] ? "live" : "unit")
    if _should_skip
  ...
  def test_direct_load_site
    setup = site_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-site", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
