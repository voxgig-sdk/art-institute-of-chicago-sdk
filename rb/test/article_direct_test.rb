...
class ArticleDirectTest < Minitest::Test
  def test_direct_list_article
    setup = article_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-article", setup[:live] ? "live" : "unit")
    if _should_skip
  ...
  def test_direct_load_article
    setup = article_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-article", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
...
def article_direct_setup(mockres)
