...
class VideoEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Video(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Video(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set ART_INSTITUTE_OF_CHICAGO_TEST_VIDEO_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    video_ref01_data_dt0_load_result = Helpers.to_map(video_ref01_data_dt0_loaded.respond_to?(:data_get) ? video_ref01_data_dt0_loaded.data_get : video_ref01_data_dt0_loaded)
...
def video_basic_setup(extra)
  ...
  entid_env_raw = ENV["ART_INSTITUTE_OF_CHICAGO_TEST_VIDEO_ENTID"]
