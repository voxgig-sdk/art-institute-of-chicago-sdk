...
class GenericPageEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.GenericPage(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.GenericPage(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = generic_page_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "generic_page." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    generic_page_ref01_data = nil
    if generic_page_ref01_data_raw.length > 0
...
def generic_page_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "generic_page", "GenericPageTestData.json")
