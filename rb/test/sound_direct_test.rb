...
class SoundDirectTest < Minitest::Test
  def test_direct_list_sound
    setup = sound_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-sound", setup[:live] ? "live" : "unit")
    if _should_skip
  ...
  def test_direct_load_sound
    setup = sound_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-sound", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
...
def sound_direct_setup(mockres)
