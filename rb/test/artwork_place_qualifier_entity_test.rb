...
class ArtworkPlaceQualifierEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.ArtworkPlaceQualifier(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.ArtworkPlaceQualifier(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set ART_INSTITUTE_OF_CHICAGO_TEST_ARTWORK_PLACE_QUALIFIER_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    artwork_place_qualifier_ref01_data_dt0_load_result = Helpers.to_map(artwork_place_qualifier_ref01_data_dt0_loaded.respond_to?(:data_get) ? artwork_place_qualifier_ref01_data_dt0_loaded.data_get : artwork_place_qualifier_ref01_data_dt0_loaded)
...
def artwork_place_qualifier_basic_setup(extra)
  ...
  entid_env_raw = ENV["ART_INSTITUTE_OF_CHICAGO_TEST_ARTWORK_PLACE_QUALIFIER_ENTID"]
