...
class ArtInstituteOfChicagoSDK
    ...
    if feature_opts
      ...
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ArtInstituteOfChicagoHelpers.to_map(feature_opts[fname])
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => ArtInstituteOfChicagoError.new(
        "#{op}_allow",
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ArtInstituteOfChicagoError.new(
        "graphql_error", "ArtInstituteOfChicagoSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
