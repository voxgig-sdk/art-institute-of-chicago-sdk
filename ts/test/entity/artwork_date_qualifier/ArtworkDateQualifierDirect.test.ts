...
describe('ArtworkDateQualifierDirect', async () => {
  ...
  test('direct-load-artwork_date_qualifier', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-artwork_date_qualifier', setup.live)) return
  ...
  test('direct-list-artwork_date_qualifier', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-artwork_date_qualifier', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['ART_INSTITUTE_OF_CHICAGO_TEST_ARTWORK_DATE_QUALIFIER_ENTID']
