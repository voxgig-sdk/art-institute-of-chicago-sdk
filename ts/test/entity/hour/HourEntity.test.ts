...
describe('HourEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'hour.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set ART_INSTITUTE_OF_CHICAGO_TEST_HOUR_ENTID JSON to run live')
      return
    }
    ...
    let hour_ref01_data = Object.values(setup.data.existing.hour)[0] as any

    // LIST
    const hour_ref01_ent = client.Hour()
    const hour_ref01_match: any = {}

    const hour_ref01_list = await hour_ref01_ent.list(hour_ref01_match)


    // LOAD
    const hour_ref01_match_dt0: any = {}
    hour_ref01_match_dt0.id = hour_ref01_data.id
    const hour_ref01_data_dt0 = await hour_ref01_ent.load(hour_ref01_match_dt0)
    assert(hour_ref01_data_dt0.id === hour_ref01_data.id)
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/hour/HourTestData.json')
  ...
  const idmapEnvVal = process.env['ART_INSTITUTE_OF_CHICAGO_TEST_HOUR_ENTID']
