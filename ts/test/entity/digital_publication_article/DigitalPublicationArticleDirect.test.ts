...
describe('DigitalPublicationArticleDirect', async () => {
  ...
  test('direct-load-digital_publication_article', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-digital_publication_article', setup.live)) return
  ...
  test('direct-list-digital_publication_article', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-digital_publication_article', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['ART_INSTITUTE_OF_CHICAGO_TEST_DIGITAL_PUBLICATION_ARTICLE_ENTID']
