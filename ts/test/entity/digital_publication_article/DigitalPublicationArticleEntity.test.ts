...
describe('DigitalPublicationArticleEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'digital_publication_article.' + op, live)) return
    }
    ...
    const digital_publication_article_ref01_list = await digital_publication_article_ref01_ent.list(digital_publication_article_ref01_match)
    ...
    const digital_publication_article_ref01_data_dt0 = await digital_publication_article_ref01_ent.load(digital_publication_article_ref01_match_dt0)
