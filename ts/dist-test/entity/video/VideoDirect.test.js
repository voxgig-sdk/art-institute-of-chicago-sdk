...
(0, node_test_1.describe)('VideoDirect', async () => {
    ...
    (0, node_test_1.test)('direct-list-video', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-video', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function directSetup(mockres) {
    ...
    const env = (0, utility_1.envOverride)({
        'ART_INSTITUTE_OF_CHICAGO_TEST_VIDEO_ENTID': {},
    ...
    if (live) {
        ...
        let idmap = env['ART_INSTITUTE_OF_CHICAGO_TEST_VIDEO_ENTID'];
