...
(0, node_test_1.describe)('TourEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['list', 'load']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'tour.' + op, live))
                return;
        }
        ...
        if (setup.syntheticOnly) {
            t.skip('live entity test uses synthetic IDs from fixture — set ART_INSTITUTE_OF_CHICAGO_TEST_TOUR_ENTID JSON to run live');
            return;
        }
        ...
        let tour_ref01_data = Object.values(setup.data.existing.tour)[0];
        // LIST
        const tour_ref01_ent = client.Tour();
        const tour_ref01_match = {};
        const tour_ref01_list = (await tour_ref01_ent.list(tour_ref01_match)).map((e) => e.data());
        // LOAD
        const tour_ref01_match_dt0 = {};
        tour_ref01_match_dt0.id = tour_ref01_data.id;
        const tour_ref01_data_dt0 = (await tour_ref01_ent.load(tour_ref01_match_dt0)).data();
        (0, node_assert_1.default)(tour_ref01_data_dt0.id === tour_ref01_data.id);
    });
});
function basicSetup(extra) {
    ...
    const entityDataFile = node_path_1.default.resolve(__dirname, '../../../../.sdk/test/entity/tour/TourTestData.json');
    ...
    let idmap = transform(['tour01', 'tour02', 'tour03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['ART_INSTITUTE_OF_CHICAGO_TEST_TOUR_ENTID'];
...
//# sourceMappingURL=TourEntity.test.js.map