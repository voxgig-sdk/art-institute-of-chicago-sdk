...
(0, node_test_1.describe)('TextEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['list', 'load']) {
            if (!live && (0, utility_1.maybeSkipControl)(t, 'entityOp', 'text.' + op, live))
                return;
        }
        ...
        let text_ref01_data = Object.values(setup.data.existing.text)[0];
        // LIST
        const text_ref01_ent = client.Text();
        const text_ref01_match = {};
        const text_ref01_list = (await text_ref01_ent.list(text_ref01_match)).map((e) => e.data());
        // LOAD
        const text_ref01_match_dt0 = {};
        text_ref01_match_dt0.id = text_ref01_data.id;
        const text_ref01_data_dt0 = (await text_ref01_ent.load(text_ref01_match_dt0)).data();
        (0, node_assert_1.default)(text_ref01_data_dt0.id === text_ref01_data.id);
    });
});
function basicSetup(extra) {
    ...
    const entityDataFile = node_path_1.default.resolve(__dirname, '../../../../.sdk/test/entity/text/TextTestData.json');
    ...
    let idmap = transform(['text01', 'text02', 'text03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
...
//# sourceMappingURL=TextEntity.test.js.map