...
The Python SDK for the ArtInstituteOfChicago API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Agent(None).list(None, None)
if err:
    raise Exception(err)
...
```python
result, err = client.Agent(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.ArtInstituteOfChicago(None).load(
    {"id": "test01"}, None
)
