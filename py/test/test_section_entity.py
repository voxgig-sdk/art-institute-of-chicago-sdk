...
class TestSectionEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Section(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Section(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _section_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "section." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set ART_INSTITUTE_OF_CHICAGO_TEST_SECTION_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        section_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.section")))
        section_ref01_data = None
        if len(section_ref01_data_raw) > 0:
        ...
        section_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(section_ref01_data_dt0_loaded))
        assert section_ref01_data_dt0_load_result is not None
        assert section_ref01_data_dt0_load_result["id"] == section_ref01_data["id"]



def _section_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/section/SectionTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "ART_INSTITUTE_OF_CHICAGO_TEST_SECTION_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("ART_INSTITUTE_OF_CHICAGO_TEST_SECTION_ENTID"))
