# ArtInstituteOfChicago SDK test runner
...
class ArtInstituteOfChicagoTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ArtInstituteOfChicagoTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ArtInstituteOfChicagoTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ArtInstituteOfChicagoTestRunner.getenv("ART_INSTITUTE_OF_CHICAGO_TEST_LIVE")
        override = ArtInstituteOfChicagoTestRunner.getenv("ART_INSTITUTE_OF_CHICAGO_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ArtInstituteOfChicagoTestRunner.getenv(key)
        ...
        explain = ArtInstituteOfChicagoTestRunner.getenv("ART_INSTITUTE_OF_CHICAGO_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["ART_INSTITUTE_OF_CHICAGO_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if ArtInstituteOfChicagoTestRunner._test_control is not None:
            return ArtInstituteOfChicagoTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ArtInstituteOfChicagoTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ArtInstituteOfChicagoTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ArtInstituteOfChicagoTestRunner.load_test_control()
...
def load_env_local():
    ArtInstituteOfChicagoTestRunner.load_env_local()


def env_override(m):
    return ArtInstituteOfChicagoTestRunner.env_override(m)


def entity_data(v):
    return ArtInstituteOfChicagoTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ArtInstituteOfChicagoTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ArtInstituteOfChicagoTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ArtInstituteOfChicagoTestRunner.load_test_control()


def live_client_options():
    return ArtInstituteOfChicagoTestRunner.live_client_options()


def live_delay_ms():
    return ArtInstituteOfChicagoTestRunner.live_delay_ms()
