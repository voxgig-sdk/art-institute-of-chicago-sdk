    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set ARTINSTITUTEOFCHICAGO_TEST_SITE_ENTID JSON to run live")
        client = setup["client"]
        ...
        site_ref01_list_result, err = site_ref01_ent.list(site_ref01_match, None)
        assert err is None
        ...
        site_ref01_data_dt0_loaded, err = site_ref01_ent.load(site_ref01_match_dt0, None)
        assert err is None
...
def _site_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("ARTINSTITUTEOFCHICAGO_TEST_SITE_ENTID"))
