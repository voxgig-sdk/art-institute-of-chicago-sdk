...
class TestVideoEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Video(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Video(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set ART_INSTITUTE_OF_CHICAGO_TEST_VIDEO_ENTID JSON to run live")
        client = setup["client"]
        ...
        video_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(video_ref01_data_dt0_loaded))
...
def _video_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("ART_INSTITUTE_OF_CHICAGO_TEST_VIDEO_ENTID"))
