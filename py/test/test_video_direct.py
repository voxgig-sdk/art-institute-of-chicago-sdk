...
class TestVideoDirect:
    ...
    def test_should_direct_list_video(self):
        setup = _video_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-video", "live" if setup["live"] else "unit")
        if _skip:
