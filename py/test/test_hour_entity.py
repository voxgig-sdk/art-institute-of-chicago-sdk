...
class TestHourEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Hour(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Hour(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _hour_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "hour." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set ART_INSTITUTE_OF_CHICAGO_TEST_HOUR_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        hour_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.hour")))
        hour_ref01_data = None
        if len(hour_ref01_data_raw) > 0:
            hour_ref01_data = helpers.to_map(hour_ref01_data_raw[0][1])

        # LIST
        hour_ref01_ent = client.Hour(None)
        hour_ref01_match = {}

        hour_ref01_list_result = hour_ref01_ent.list(hour_ref01_match, None)
        assert isinstance(hour_ref01_list_result, list)

        # LOAD
        hour_ref01_match_dt0 = {
            "id": hour_ref01_data["id"],
        }
        hour_ref01_data_dt0_loaded = hour_ref01_ent.load(hour_ref01_match_dt0, None)
        hour_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(hour_ref01_data_dt0_loaded))
        assert hour_ref01_data_dt0_load_result is not None
        assert hour_ref01_data_dt0_load_result["id"] == hour_ref01_data["id"]



def _hour_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/hour/HourTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "ART_INSTITUTE_OF_CHICAGO_TEST_HOUR_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("ART_INSTITUTE_OF_CHICAGO_TEST_HOUR_ENTID"))
