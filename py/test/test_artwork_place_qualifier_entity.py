...
class TestArtworkPlaceQualifierEntity:

    def test_should_create_instance(self):
        testsdk = ArtInstituteOfChicagoSDK.test(None, None)
        ent = testsdk.ArtworkPlaceQualifier(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.ArtworkPlaceQualifier(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.ArtworkPlaceQualifier(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _artwork_place_qualifier_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "artwork_place_qualifier." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set ART_INSTITUTE_OF_CHICAGO_TEST_ARTWORK_PLACE_QUALIFIER_ENTID JSON to run live")
        client = setup["client"]
        ...
        artwork_place_qualifier_ref01_data = None
        if len(artwork_place_qualifier_ref01_data_raw) > 0:
        ...
        artwork_place_qualifier_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(artwork_place_qualifier_ref01_data_dt0_loaded))
...
def _artwork_place_qualifier_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/artwork_place_qualifier/ArtworkPlaceQualifierTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "ART_INSTITUTE_OF_CHICAGO_TEST_ARTWORK_PLACE_QUALIFIER_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("ART_INSTITUTE_OF_CHICAGO_TEST_ARTWORK_PLACE_QUALIFIER_ENTID"))
