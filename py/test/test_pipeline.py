...
def _client():
    return ArtInstituteOfChicagoSDK.test(None, None)
...
def _full_spec():
    return ArtInstituteOfChicagoSpec({
        "base": "http://h",
        "prefix": "",
        "suffix": "",
        "path": "a",
...
def _resp(status, data=None, headers=None):
    ...
    return ArtInstituteOfChicagoResponse({
        "status": status,
...
class TestMakePointAndMakeSpec:
    ...
    def test_make_spec_short_circuits_a_feature_supplied_spec(self):
        client = _client()
        ctx = _ctx(client, opname="load")
        preset = ArtInstituteOfChicagoSpec({"method": "GET"})
        ctx.out["spec"] = preset
...
class TestMakeResult:
    ...
    def test_list_op_wraps_resdata_into_entity_instances(self):
        ...
        ctx.entity = entity
        ctx.spec = _full_spec()
        ctx.result = ArtInstituteOfChicagoResult({"resdata": [{"a": 1}, {"a": 2}]})
    ...
    def test_an_empty_list_yields_an_empty_resdata_array(self):
        ...
        ctx.spec = _full_spec()
        ctx.result = ArtInstituteOfChicagoResult({"resdata": []})
        result, err = client._utility.make_result(ctx)
        assert err is None
    ...
    def test_short_circuits_on_a_preset_result(self):
        client = _client()
        ctx = _ctx(client)
        preset = ArtInstituteOfChicagoResult({"ok": True})
...
class TestMakeRequest:
    ...
    def test_short_circuits_a_feature_supplied_request(self):
        client = _client()
        ctx = _ctx(client)
        preset = ArtInstituteOfChicagoResponse({"status": 201, "statusText": "OK"})
...
class TestMakeErrorAndDone:
    ...
    def test_done_returns_resdata_on_success(self):
        client = _client()
        ctx = _ctx(client)
        ctx.result = ArtInstituteOfChicagoResult({"ok": True, "resdata": 42})
        assert client._utility.done(ctx) == 42

    def test_done_raises_the_error_when_not_ok(self):
        client = _client()
        ctx = _ctx(client)
        ctx.result = ArtInstituteOfChicagoResult({"ok": False})
        with pytest.raises(ArtInstituteOfChicagoError):
            client._utility.done(ctx)

    def test_done_cleans_ctrl_explain_on_success(self):
        client = _client()
        ctx = _ctx(client, ctrl={"explain": {"result": {"err": "x"}}})
        ctx.result = ArtInstituteOfChicagoResult({"ok": True, "resdata": 7})
    ...
    def test_make_error_returns_resdata_when_throw_is_disabled(self):
        ...
        ctx.result = ArtInstituteOfChicagoResult({"ok": False, "resdata": "fallback"})
...
class TestPrepareAuth:
    ...
    def _auth_ctx(self, client, options, headers):
        ...
        ctx.spec = None if headers is None else ArtInstituteOfChicagoSpec(
            {"headers": headers})
        return ctx
...
class TestResultHelpers:
    ...
    def test_result_headers_with_no_headers_yields_an_empty_map(self):
        client = _client()
        ctx = _ctx(client)
        ctx.response = ArtInstituteOfChicagoResponse({"status": 200})
        ctx.result = ArtInstituteOfChicagoResult({})
