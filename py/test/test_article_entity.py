...
class TestArticleEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Article(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Article(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _article_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "article." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set ART_INSTITUTE_OF_CHICAGO_TEST_ARTICLE_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        article_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.article")))
        article_ref01_data = None
        if len(article_ref01_data_raw) > 0:
...
def _article_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/article/ArticleTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("ART_INSTITUTE_OF_CHICAGO_TEST_ARTICLE_ENTID"))
