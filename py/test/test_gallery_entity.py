...
class TestGalleryEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Gallery(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Gallery(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set ART_INSTITUTE_OF_CHICAGO_TEST_GALLERY_ENTID JSON to run live")
        client = setup["client"]
        ...
        gallery_ref01_list_result = gallery_ref01_ent.list(gallery_ref01_match, None)
        ...
        gallery_ref01_data_dt0_loaded = gallery_ref01_ent.load(gallery_ref01_match_dt0, None)
        gallery_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(gallery_ref01_data_dt0_loaded))
...
def _gallery_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("ART_INSTITUTE_OF_CHICAGO_TEST_GALLERY_ENTID"))
