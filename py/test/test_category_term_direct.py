...
class TestCategoryTermDirect:

    def test_should_direct_list_category_term(self):
        setup = _category_term_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-category_term", "live" if setup["live"] else "unit")
        if _skip:
    ...
    def test_should_direct_load_category_term(self):
        setup = _category_term_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-category_term", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _category_term_direct_setup(mockres):
