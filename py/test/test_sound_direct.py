...
class TestSoundDirect:

    def test_should_direct_list_sound(self):
        setup = _sound_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-sound", "live" if setup["live"] else "unit")
        if _skip:
    ...
    def test_should_direct_load_sound(self):
        setup = _sound_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-sound", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _sound_direct_setup(mockres):
