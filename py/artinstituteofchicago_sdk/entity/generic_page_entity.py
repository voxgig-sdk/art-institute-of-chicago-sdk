...
from artinstituteofchicago_sdk.artinstituteofchicago_types import (
    GenericPage,
    GenericPageLoadMatch,
    GenericPageListMatch,
)
...
class GenericPageEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return GenericPageEntity(self._client, opts)
    ...
    def match_get(self) -> GenericPage:
    ...
    def load(self, reqmatch=None, ctrl=None) -> GenericPage:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.GenericPage().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[GenericPage]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.GenericPage().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
