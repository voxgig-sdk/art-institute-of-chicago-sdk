...
class EventOccurrenceEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return EventOccurrenceEntity(self._client, opts)
    ...
    def data_get(self) -> EventOccurrence:
    ...
    def match_get(self) -> EventOccurrence:
    ...
    def load(self, reqmatch=None, ctrl=None) -> EventOccurrence:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.EventOccurrence().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[EventOccurrence]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.EventOccurrence().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
