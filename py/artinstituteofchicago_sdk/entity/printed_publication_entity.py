...
class PrintedPublicationEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PrintedPublicationEntity(self._client, opts)
    ...
    def data_get(self) -> PrintedPublication:
    ...
    def match_get(self) -> PrintedPublication:
    ...
    def load(self, reqmatch=None, ctrl=None) -> PrintedPublication:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.PrintedPublication().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[PrintedPublication]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.PrintedPublication().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
