...
from artinstituteofchicago_sdk.artinstituteofchicago_types import (
    PressRelease,
    PressReleaseLoadMatch,
    PressReleaseListMatch,
)
...
class PressReleaseEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PressReleaseEntity(self._client, opts)
    ...
    def data_get(self) -> PressRelease:
    ...
    def match_get(self) -> PressRelease:
    ...
    def load(self, reqmatch=None, ctrl=None) -> PressRelease:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.PressRelease().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[PressRelease]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.PressRelease().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
