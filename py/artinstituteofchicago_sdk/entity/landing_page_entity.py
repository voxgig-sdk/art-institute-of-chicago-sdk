...
from artinstituteofchicago_sdk.artinstituteofchicago_types import (
    LandingPage,
    LandingPageLoadMatch,
    LandingPageListMatch,
)
...
class LandingPageEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return LandingPageEntity(self._client, opts)
    ...
    def match_get(self) -> LandingPage:
    ...
    def load(self, reqmatch=None, ctrl=None) -> LandingPage:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.LandingPage().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[LandingPage]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.LandingPage().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
