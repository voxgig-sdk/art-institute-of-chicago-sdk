...
from artinstituteofchicago_sdk.artinstituteofchicago_types import (
    DigitalPublication,
    DigitalPublicationLoadMatch,
    DigitalPublicationListMatch,
)
...
class DigitalPublicationEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DigitalPublicationEntity(self._client, opts)
    ...
    def data_get(self) -> DigitalPublication:
    ...
    def match_get(self) -> DigitalPublication:
    ...
    def load(self, reqmatch=None, ctrl=None) -> DigitalPublication:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.DigitalPublication().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[DigitalPublication]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.DigitalPublication().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
