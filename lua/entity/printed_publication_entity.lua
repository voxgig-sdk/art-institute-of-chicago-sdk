...
local PrintedPublicationEntity = {}
PrintedPublicationEntity.__index = PrintedPublicationEntity


function PrintedPublicationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PrintedPublicationEntity)
  self._name = "printed_publication"
  self._client = client
...
function PrintedPublicationEntity:get_name()
  return self._name
end


function PrintedPublicationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PrintedPublicationEntity.new(self._client, opts)
end
...
function PrintedPublicationEntity:mark_deleted()
  self._deleted = true
end


function PrintedPublicationEntity:deleted()
  return true == self._deleted
end


function PrintedPublicationEntity:data_set(args)
  if args ~= nil then
...
function PrintedPublicationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PrintedPublicationEntity:match_set(args)
  if args ~= nil then
...
function PrintedPublicationEntity:match_get()
...
function PrintedPublicationEntity:stream(action, args, callopts)
...
---@param reqmatch PrintedPublicationLoadMatch
---@param ctrl? table
---@return PrintedPublication
---@return string? err
function PrintedPublicationEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch PrintedPublicationListMatch
---@param ctrl? table
---@return PrintedPublication[]
---@return string? err
function PrintedPublicationEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function PrintedPublicationEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return PrintedPublicationEntity
