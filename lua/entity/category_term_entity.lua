...
local CategoryTermEntity = {}
CategoryTermEntity.__index = CategoryTermEntity


function CategoryTermEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CategoryTermEntity)
  self._name = "category_term"
  self._client = client
...
function CategoryTermEntity:get_name()
  return self._name
end


function CategoryTermEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CategoryTermEntity.new(self._client, opts)
end
...
function CategoryTermEntity:mark_deleted()
  self._deleted = true
end


function CategoryTermEntity:deleted()
  return true == self._deleted
end


function CategoryTermEntity:data_set(args)
  if args ~= nil then
...
function CategoryTermEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CategoryTermEntity:match_set(args)
  if args ~= nil then
...
function CategoryTermEntity:match_get()
...
function CategoryTermEntity:stream(action, args, callopts)
...
function CategoryTermEntity:_run_op(ctx, post_done)
  local utility = self._utility
