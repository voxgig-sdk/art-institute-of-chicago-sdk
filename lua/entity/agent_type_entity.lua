...
local AgentTypeEntity = {}
AgentTypeEntity.__index = AgentTypeEntity


function AgentTypeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AgentTypeEntity)
  self._name = "agent_type"
  self._client = client
...
function AgentTypeEntity:get_name()
  return self._name
end


function AgentTypeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AgentTypeEntity.new(self._client, opts)
end
...
function AgentTypeEntity:mark_deleted()
  self._deleted = true
end


function AgentTypeEntity:deleted()
  return true == self._deleted
end


function AgentTypeEntity:data_set(args)
  if args ~= nil then
...
function AgentTypeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AgentTypeEntity:match_set(args)
  if args ~= nil then
...
function AgentTypeEntity:match_get()
...
function AgentTypeEntity:stream(action, args, callopts)
...
function AgentTypeEntity:_run_op(ctx, post_done)
  local utility = self._utility
