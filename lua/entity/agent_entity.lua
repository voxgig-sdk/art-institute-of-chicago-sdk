...
function AgentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AgentEntity)
  self._name = "agent"
  self._client = client
...
function AgentEntity:get_name()
  return self._name
end


function AgentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AgentEntity.new(self._client, opts)
end
...
function AgentEntity:mark_deleted()
  self._deleted = true
end


function AgentEntity:deleted()
  return true == self._deleted
end


function AgentEntity:data_set(args)
  if args ~= nil then
...
function AgentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AgentEntity:match_set(args)
  if args ~= nil then
...
function AgentEntity:match_get()
...
function AgentEntity:stream(action, args, callopts)
...
---@param reqmatch AgentLoadMatch
---@param ctrl? table
---@return Agent
---@return string? err
function AgentEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch AgentListMatch
---@param ctrl? table
---@return Agent[]
---@return string? err
function AgentEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function AgentEntity:_run_op(ctx, post_done)
  local utility = self._utility
