...
local ArticleEntity = {}
ArticleEntity.__index = ArticleEntity


function ArticleEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ArticleEntity)
  self._name = "article"
  self._client = client
...
function ArticleEntity:get_name()
  return self._name
end


function ArticleEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ArticleEntity.new(self._client, opts)
end
...
function ArticleEntity:mark_deleted()
  self._deleted = true
end


function ArticleEntity:deleted()
  return true == self._deleted
end


function ArticleEntity:data_set(args)
  if args ~= nil then
...
function ArticleEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ArticleEntity:match_set(args)
  if args ~= nil then
...
function ArticleEntity:match_get()
...
function ArticleEntity:stream(action, args, callopts)
...
function ArticleEntity:_run_op(ctx, post_done)
  local utility = self._utility
