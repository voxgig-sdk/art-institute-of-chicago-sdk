-- ArtInstituteOfChicago SDK AgentRole entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local AgentRoleEntity = {}
AgentRoleEntity.__index = AgentRoleEntity


function AgentRoleEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AgentRoleEntity)
  self._name = "agent_role"
  self._client = client
...
function AgentRoleEntity:get_name()
  return self._name
end


function AgentRoleEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AgentRoleEntity.new(self._client, opts)
end
...
function AgentRoleEntity:mark_deleted()
  self._deleted = true
end


function AgentRoleEntity:deleted()
  return true == self._deleted
end


function AgentRoleEntity:data_set(args)
  if args ~= nil then
...
function AgentRoleEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AgentRoleEntity:match_set(args)
  if args ~= nil then
...
function AgentRoleEntity:match_get()
...
function AgentRoleEntity:stream(action, args, callopts)
...
function AgentRoleEntity:_run_op(ctx, post_done)
  local utility = self._utility
