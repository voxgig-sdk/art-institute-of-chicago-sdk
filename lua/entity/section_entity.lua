...
local SectionEntity = {}
SectionEntity.__index = SectionEntity


function SectionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SectionEntity)
  self._name = "section"
  self._client = client
...
function SectionEntity:get_name()
  return self._name
end


function SectionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SectionEntity.new(self._client, opts)
end
...
function SectionEntity:mark_deleted()
  self._deleted = true
end


function SectionEntity:deleted()
  return true == self._deleted
end


function SectionEntity:data_set(args)
  if args ~= nil then
...
function SectionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SectionEntity:match_set(args)
  if args ~= nil then
...
function SectionEntity:match_get()
...
function SectionEntity:stream(action, args, callopts)
...
function SectionEntity:_run_op(ctx, post_done)
  local utility = self._utility
