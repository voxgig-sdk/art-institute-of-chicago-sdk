...
local PressReleaseEntity = {}
PressReleaseEntity.__index = PressReleaseEntity


function PressReleaseEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PressReleaseEntity)
  self._name = "press_release"
  self._client = client
...
function PressReleaseEntity:get_name()
  return self._name
end


function PressReleaseEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PressReleaseEntity.new(self._client, opts)
end
...
function PressReleaseEntity:mark_deleted()
  self._deleted = true
end


function PressReleaseEntity:deleted()
  return true == self._deleted
end


function PressReleaseEntity:data_set(args)
  if args ~= nil then
...
function PressReleaseEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PressReleaseEntity:match_set(args)
  if args ~= nil then
...
function PressReleaseEntity:match_get()
...
function PressReleaseEntity:stream(action, args, callopts)
...
---@param reqmatch PressReleaseLoadMatch
---@param ctrl? table
---@return PressRelease
---@return string? err
function PressReleaseEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch PressReleaseListMatch
---@param ctrl? table
---@return PressRelease[]
---@return string? err
function PressReleaseEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function PressReleaseEntity:_run_op(ctx, post_done)
  local utility = self._utility
