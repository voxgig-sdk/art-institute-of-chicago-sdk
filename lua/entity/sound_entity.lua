...
function SoundEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SoundEntity)
  self._name = "sound"
  self._client = client
...
function SoundEntity:get_name()
  return self._name
end


function SoundEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SoundEntity.new(self._client, opts)
end
...
function SoundEntity:mark_deleted()
  self._deleted = true
end


function SoundEntity:deleted()
  return true == self._deleted
end


function SoundEntity:data_set(args)
  if args ~= nil then
...
function SoundEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SoundEntity:match_set(args)
  if args ~= nil then
...
function SoundEntity:match_get()
...
function SoundEntity:stream(action, args, callopts)
...
---@param reqmatch SoundLoadMatch
---@param ctrl? table
---@return Sound
---@return string? err
function SoundEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch SoundListMatch
---@param ctrl? table
---@return Sound[]
---@return string? err
function SoundEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function SoundEntity:_run_op(ctx, post_done)
  local utility = self._utility
