...
local EventOccurrenceEntity = {}
EventOccurrenceEntity.__index = EventOccurrenceEntity


function EventOccurrenceEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EventOccurrenceEntity)
  self._name = "event_occurrence"
  self._client = client
...
function EventOccurrenceEntity:get_name()
  return self._name
end


function EventOccurrenceEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EventOccurrenceEntity.new(self._client, opts)
end
...
function EventOccurrenceEntity:mark_deleted()
  self._deleted = true
end


function EventOccurrenceEntity:deleted()
  return true == self._deleted
end


function EventOccurrenceEntity:data_set(args)
  if args ~= nil then
...
function EventOccurrenceEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EventOccurrenceEntity:match_set(args)
  if args ~= nil then
...
function EventOccurrenceEntity:match_get()
...
function EventOccurrenceEntity:stream(action, args, callopts)
...
function EventOccurrenceEntity:_run_op(ctx, post_done)
  local utility = self._utility
