...
local HourEntity = {}
HourEntity.__index = HourEntity


function HourEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, HourEntity)
  self._name = "hour"
  self._client = client
...
function HourEntity:get_name()
  return self._name
end


function HourEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return HourEntity.new(self._client, opts)
end
...
function HourEntity:mark_deleted()
  self._deleted = true
end


function HourEntity:deleted()
  return true == self._deleted
end


function HourEntity:data_set(args)
  if args ~= nil then
...
function HourEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function HourEntity:match_set(args)
  if args ~= nil then
...
function HourEntity:stream(action, args, callopts)
...
function HourEntity:_run_op(ctx, post_done)
  local utility = self._utility
