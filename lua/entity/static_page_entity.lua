...
local StaticPageEntity = {}
StaticPageEntity.__index = StaticPageEntity


function StaticPageEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StaticPageEntity)
  self._name = "static_page"
  self._client = client
...
function StaticPageEntity:get_name()
  return self._name
end


function StaticPageEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StaticPageEntity.new(self._client, opts)
end
...
function StaticPageEntity:mark_deleted()
  self._deleted = true
end


function StaticPageEntity:deleted()
  return true == self._deleted
end


function StaticPageEntity:data_set(args)
  if args ~= nil then
...
function StaticPageEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StaticPageEntity:match_set(args)
  if args ~= nil then
...
function StaticPageEntity:match_get()
...
function StaticPageEntity:stream(action, args, callopts)
...
function StaticPageEntity:_run_op(ctx, post_done)
  local utility = self._utility
