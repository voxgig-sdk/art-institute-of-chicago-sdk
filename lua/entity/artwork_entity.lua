...
local ArtworkEntity = {}
ArtworkEntity.__index = ArtworkEntity


function ArtworkEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ArtworkEntity)
  self._name = "artwork"
  self._client = client
...
function ArtworkEntity:get_name()
  return self._name
end


function ArtworkEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ArtworkEntity.new(self._client, opts)
end
...
function ArtworkEntity:mark_deleted()
  self._deleted = true
end


function ArtworkEntity:deleted()
  return true == self._deleted
end


function ArtworkEntity:data_set(args)
  if args ~= nil then
...
function ArtworkEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ArtworkEntity:match_set(args)
  if args ~= nil then
...
function ArtworkEntity:match_get()
...
function ArtworkEntity:stream(action, args, callopts)
...
---@param reqmatch ArtworkLoadMatch
---@param ctrl? table
---@return Artwork
---@return string? err
function ArtworkEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch ArtworkListMatch
---@param ctrl? table
---@return Artwork[]
---@return string? err
function ArtworkEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function ArtworkEntity:_run_op(ctx, post_done)
  local utility = self._utility
