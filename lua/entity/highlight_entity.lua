...
HighlightEntity.__index = HighlightEntity


function HighlightEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, HighlightEntity)
...
function HighlightEntity:get_name()
  return self._name
end


function HighlightEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return HighlightEntity.new(self._client, opts)
end
...
function HighlightEntity:mark_deleted()
  self._deleted = true
end


function HighlightEntity:deleted()
  return true == self._deleted
end


function HighlightEntity:data_set(args)
  if args ~= nil then
...
function HighlightEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function HighlightEntity:match_set(args)
  if args ~= nil then
...
function HighlightEntity:match_get()
...
function HighlightEntity:stream(action, args, callopts)
...
function HighlightEntity:_run_op(ctx, post_done)
  local utility = self._utility
