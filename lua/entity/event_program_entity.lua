...
local EventProgramEntity = {}
EventProgramEntity.__index = EventProgramEntity


function EventProgramEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EventProgramEntity)
  self._name = "event_program"
  self._client = client
...
function EventProgramEntity:get_name()
  return self._name
end


function EventProgramEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EventProgramEntity.new(self._client, opts)
end
...
function EventProgramEntity:mark_deleted()
  self._deleted = true
end


function EventProgramEntity:deleted()
  return true == self._deleted
end


function EventProgramEntity:data_set(args)
  if args ~= nil then
...
function EventProgramEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EventProgramEntity:match_set(args)
  if args ~= nil then
...
function EventProgramEntity:match_get()
...
function EventProgramEntity:stream(action, args, callopts)
...
function EventProgramEntity:_run_op(ctx, post_done)
  local utility = self._utility
