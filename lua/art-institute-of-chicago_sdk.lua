...
require("art-institute-of-chicago_types")
...
function ArtInstituteOfChicagoSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function ArtInstituteOfChicagoSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "ArtInstituteOfChicagoSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function ArtInstituteOfChicagoSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function ArtInstituteOfChicagoSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "ArtInstituteOfChicagoSDK: graphql: " .. msg
    res.graphql = errors
  end
