...
describe("AgentEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set ARTINSTITUTEOFCHICAGO_TEST_AGENT_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local agent_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.agent")))
    local agent_ref01_data = nil
    if #agent_ref01_data_raw > 0 then
      agent_ref01_data = helpers.to_map(agent_ref01_data_raw[1][2])
    end
    ...
    assert.is_nil(err)
    local agent_ref01_data_dt0_load_result = helpers.to_map(agent_ref01_data_dt0_loaded)
...
function agent_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("ARTINSTITUTEOFCHICAGO_TEST_AGENT_ENTID")
  local idmap_overridden = entid_env_raw ~= nil and entid_env_raw:match("^%s*{") ~= nil

  local env = runner.env_override({
    ["ARTINSTITUTEOFCHICAGO_TEST_AGENT_ENTID"] = idmap,
    ["ARTINSTITUTEOFCHICAGO_TEST_LIVE"] = "FALSE",
    ["ARTINSTITUTEOFCHICAGO_TEST_EXPLAIN"] = "FALSE",
  })

  local idmap_resolved = helpers.to_map(
    env["ARTINSTITUTEOFCHICAGO_TEST_AGENT_ENTID"])
