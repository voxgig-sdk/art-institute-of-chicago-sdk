...
describe("SoundDirect", function()
  it("should direct-list-sound", function()
    local setup = sound_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-sound", setup.live and "live" or "unit")
  ...
  it("should direct-load-sound", function()
    local setup = sound_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-sound", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function sound_direct_setup(mockres)
