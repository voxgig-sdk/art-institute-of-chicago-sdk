...
describe("HighlightDirect", function()
  it("should direct-list-highlight", function()
    local setup = highlight_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-highlight", setup.live and "live" or "unit")
  ...
  it("should direct-load-highlight", function()
    local setup = highlight_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-highlight", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function highlight_direct_setup(mockres)
