...
describe("HourDirect", function()
  it("should direct-list-hour", function()
    local setup = hour_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-hour", setup.live and "live" or "unit")
  ...
  it("should direct-load-hour", function()
    local setup = hour_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-hour", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function hour_direct_setup(mockres)
