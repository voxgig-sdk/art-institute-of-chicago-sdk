...
describe("EventProgramDirect", function()
  it("should direct-list-event_program", function()
    local setup = event_program_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-event_program", setup.live and "live" or "unit")
  ...
  it("should direct-load-event_program", function()
    local setup = event_program_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-event_program", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function event_program_direct_setup(mockres)
