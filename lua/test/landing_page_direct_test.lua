...
describe("LandingPageDirect", function()
  it("should direct-list-landing_page", function()
    local setup = landing_page_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-landing_page", setup.live and "live" or "unit")
  ...
  it("should direct-load-landing_page", function()
    local setup = landing_page_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-landing_page", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function landing_page_direct_setup(mockres)
