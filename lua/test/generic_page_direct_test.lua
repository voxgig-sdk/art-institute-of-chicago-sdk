...
describe("GenericPageDirect", function()
  it("should direct-list-generic_page", function()
    local setup = generic_page_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-generic_page", setup.live and "live" or "unit")
  ...
  it("should direct-load-generic_page", function()
    local setup = generic_page_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-generic_page", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function generic_page_direct_setup(mockres)
  runner.load_env_local()

  local calls = {}

  local env = runner.env_override({
    ["ARTINSTITUTEOFCHICAGO_TEST_GENERIC_PAGE_ENTID"] = {},
