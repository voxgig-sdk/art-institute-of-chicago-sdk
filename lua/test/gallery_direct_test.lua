...
describe("GalleryDirect", function()
  it("should direct-list-gallery", function()
    local setup = gallery_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-gallery", setup.live and "live" or "unit")
  ...
  it("should direct-load-gallery", function()
    local setup = gallery_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-gallery", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function gallery_direct_setup(mockres)
