...
describe("AgentRoleDirect", function()
  it("should direct-list-agent_role", function()
    local setup = agent_role_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-agent_role", setup.live and "live" or "unit")
  ...
  it("should direct-load-agent_role", function()
    local setup = agent_role_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-agent_role", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function agent_role_direct_setup(mockres)
