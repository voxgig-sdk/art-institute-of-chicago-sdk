...
describe("SectionDirect", function()
  it("should direct-list-section", function()
    local setup = section_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-section", setup.live and "live" or "unit")
  ...
  it("should direct-load-section", function()
    local setup = section_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-section", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function section_direct_setup(mockres)
