...
describe("CategoryTermDirect", function()
  it("should direct-list-category_term", function()
    local setup = category_term_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-category_term", setup.live and "live" or "unit")
  ...
  it("should direct-load-category_term", function()
    local setup = category_term_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-category_term", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function category_term_direct_setup(mockres)
