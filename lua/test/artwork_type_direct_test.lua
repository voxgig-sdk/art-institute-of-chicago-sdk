...
describe("ArtworkTypeDirect", function()
  it("should direct-list-artwork_type", function()
    local setup = artwork_type_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-artwork_type", setup.live and "live" or "unit")
  ...
  it("should direct-load-artwork_type", function()
    local setup = artwork_type_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-artwork_type", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function artwork_type_direct_setup(mockres)
