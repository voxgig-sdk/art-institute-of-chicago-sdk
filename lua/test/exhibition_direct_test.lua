...
describe("ExhibitionDirect", function()
  it("should direct-list-exhibition", function()
    local setup = exhibition_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-exhibition", setup.live and "live" or "unit")
  ...
  it("should direct-load-exhibition", function()
    local setup = exhibition_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-exhibition", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function exhibition_direct_setup(mockres)
