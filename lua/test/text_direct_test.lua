...
describe("TextDirect", function()
  it("should direct-list-text", function()
    local setup = text_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-text", setup.live and "live" or "unit")
  ...
  it("should direct-load-text", function()
    local setup = text_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-text", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function text_direct_setup(mockres)
