...
describe("ArticleDirect", function()
  it("should direct-list-article", function()
    local setup = article_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-article", setup.live and "live" or "unit")
  ...
  it("should direct-load-article", function()
    local setup = article_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-article", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function article_direct_setup(mockres)
