...
describe("ExhibitionEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Exhibition(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Exhibition(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set ART_INSTITUTE_OF_CHICAGO_TEST_EXHIBITION_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    local exhibition_ref01_data_dt0_load_result = helpers.to_map(type(exhibition_ref01_data_dt0_loaded) == 'table' and exhibition_ref01_data_dt0_loaded.data_get and exhibition_ref01_data_dt0_loaded:data_get() or exhibition_ref01_data_dt0_loaded)
...
function exhibition_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("ART_INSTITUTE_OF_CHICAGO_TEST_EXHIBITION_ENTID")
