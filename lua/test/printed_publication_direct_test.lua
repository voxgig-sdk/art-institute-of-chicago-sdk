-- PrintedPublication direct test
...
describe("PrintedPublicationDirect", function()
  it("should direct-list-printed_publication", function()
    local setup = printed_publication_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-printed_publication", setup.live and "live" or "unit")
  ...
  it("should direct-load-printed_publication", function()
    local setup = printed_publication_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-printed_publication", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function printed_publication_direct_setup(mockres)
