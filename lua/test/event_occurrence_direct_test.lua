...
describe("EventOccurrenceDirect", function()
  it("should direct-list-event_occurrence", function()
    local setup = event_occurrence_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-event_occurrence", setup.live and "live" or "unit")
  ...
  it("should direct-load-event_occurrence", function()
    local setup = event_occurrence_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-event_occurrence", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function event_occurrence_direct_setup(mockres)
