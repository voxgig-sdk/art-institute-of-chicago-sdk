...
describe("PublicationDirect", function()
  ...
  it("should direct-load-publication", function()
    local setup = publication_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-publication", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
