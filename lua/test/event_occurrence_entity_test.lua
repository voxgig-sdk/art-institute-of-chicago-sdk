...
describe("EventOccurrenceEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:EventOccurrence(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:EventOccurrence(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set ART_INSTITUTE_OF_CHICAGO_TEST_EVENT_OCCURRENCE_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    local event_occurrence_ref01_data_dt0_load_result = helpers.to_map(type(event_occurrence_ref01_data_dt0_loaded) == 'table' and event_occurrence_ref01_data_dt0_loaded.data_get and event_occurrence_ref01_data_dt0_loaded:data_get() or event_occurrence_ref01_data_dt0_loaded)
...
function event_occurrence_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("ART_INSTITUTE_OF_CHICAGO_TEST_EVENT_OCCURRENCE_ENTID")
