...
describe("PressReleaseDirect", function()
  it("should direct-list-press_release", function()
    local setup = press_release_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-press_release", setup.live and "live" or "unit")
  ...
  it("should direct-load-press_release", function()
    local setup = press_release_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-press_release", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function press_release_direct_setup(mockres)
