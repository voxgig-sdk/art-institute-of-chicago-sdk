...
class OmniSmokeTest < Minitest::Test
  ...
  def smoke_pack
    runner = ArtInstituteOfChicagoOmni.make_runner(SMOKE_SPEC, ArtInstituteOfChicagoSDK.test(nil, nil))
    runner.call("smoke")
  end
  ...
  def test_runset_fails_a_wrong_result_with_omnierror
    pack = smoke_pack
    caught = assert_raises(ArtInstituteOfChicagoOmni::OmniError) do
