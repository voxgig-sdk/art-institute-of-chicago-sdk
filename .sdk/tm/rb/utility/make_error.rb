...
module ArtInstituteOfChicagoUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = ArtInstituteOfChicagoContext.new({}, nil)
    end
    op = ctx.op || ArtInstituteOfChicagoOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || ArtInstituteOfChicagoResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(ArtInstituteOfChicagoError) ? err.msg : err.to_s
    ...
    sdk_err = ArtInstituteOfChicagoError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(ArtInstituteOfChicagoError)
