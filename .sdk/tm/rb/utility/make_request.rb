...
module ArtInstituteOfChicagoUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ArtInstituteOfChicagoResponse.new({})
    ...
    if fetch_err
      ...
      response = ArtInstituteOfChicagoResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ArtInstituteOfChicagoResponse.new(fetched)
    else
