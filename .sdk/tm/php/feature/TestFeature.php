...
// ArtInstituteOfChicago SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class ArtInstituteOfChicagoTestFeature extends ArtInstituteOfChicagoBaseFeature
{
    ...
    public function init(ArtInstituteOfChicagoContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (ArtInstituteOfChicagoContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = ArtInstituteOfChicagoParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (ArtInstituteOfChicagoContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(ArtInstituteOfChicagoContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = ArtInstituteOfChicagoParam::call($ctx, $k);
