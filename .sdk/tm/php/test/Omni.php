...
//    only - a typed ArtInstituteOfChicagoContext there would make every ctx
...
final class ArtInstituteOfChicagoOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new ArtInstituteOfChicagoContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new ArtInstituteOfChicagoSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new ArtInstituteOfChicagoResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new ArtInstituteOfChicagoError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new ArtInstituteOfChicagoResponse($clean['response']);
    ...
    public static function err_from_map($m): ?ArtInstituteOfChicagoError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new ArtInstituteOfChicagoError(is_string($code) ? $code : '', $msg);
    }
