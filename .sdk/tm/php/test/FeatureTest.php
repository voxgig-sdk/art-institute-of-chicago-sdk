...
// ArtInstituteOfChicago SDK feature test
//
...
class FtCtrl extends ArtInstituteOfChicagoControl
{
...
class FtClient
{
    ...
    private ArtInstituteOfChicagoUtility $utility;

    public function __construct(ArtInstituteOfChicagoUtility $utility, array $options)
    {
    ...
    public function get_utility(): ArtInstituteOfChicagoUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ArtInstituteOfChicagoContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ArtInstituteOfChicagoContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ArtInstituteOfChicagoContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ArtInstituteOfChicagoConfig::make_feature($name);
    ...
    private static function buildUrl(ArtInstituteOfChicagoSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ArtInstituteOfChicagoError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ArtInstituteOfChicagoResponse($fetched);
        }
    ...
    private function fail(ArtInstituteOfChicagoContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ArtInstituteOfChicagoContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
