...
func NewContext(ctxmap map[string]any, basectx *Context) *Context {
	...
	if c := getCtxProp(ctxmap, "client"); c != nil {
		if sdk, ok := c.(*ArtInstituteOfChicagoSDK); ok {
			ctx.Client = sdk
		}
	}
...
func (ctx *Context) MakeError(code string, msg string) *ArtInstituteOfChicagoError {
	return NewArtInstituteOfChicagoError(code, msg, ctx)
}
